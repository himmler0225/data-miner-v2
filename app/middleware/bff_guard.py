...
async def _send_404(send: Send) -> None:
    await send({'type': 'http.response.start', 'status': 404, 'headers': [[b'content-length', b'0'], [b'cache-control', b'no-store']]})
...
class BffGuardMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if token != BFF_CLIENT_TOKEN:
            logger.warning('Blocked unauthenticated BFF access', extra={'extra_data': {'path': path, 'method': scope.get('method', '')}})
            await _send_404(send)
            return
        await self.app(scope, receive, send)
