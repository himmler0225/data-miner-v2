    ...
    async def dispatch(self, request: Request, call_next: callable) -> Response:
        ...
        if path not in _SKIP_PATHS:
            ...
            if _is_browser_ua(ua):
                snapshot: ClientSnapshot = {'ip': _extract_ip(request), 'user_agent': ua, 'accept_language': request.headers.get('Accept-Language', ''), 'accept_encoding': request.headers.get('Accept-Encoding', ''), 'ts': time.time()}
