...
from app.config.proxy_config import DEFAULT_COUNTRY, DIRECT_PROXY_TTL, PROXY_API_TTL, PUBLIC_IP, TIKTOK_COUNTRY
...
class _DirectPool:
    def __init__(self, proxies: list[str], ttl: int = DIRECT_PROXY_TTL) -> None:
        self._ttl = ttl
...
def _parse_provider(raw: dict) -> DirectProvider | ApiProvider | None:
...
def _parse_pool_entry(raw: dict) -> PoolConfig | None:
    ...
    if not countries:
        return None
    return PoolConfig(key=str(raw.get("key") or "_".join(countries).lower()), countries=countries, provider=provider)


def _resolve_query(query: dict[str, str]) -> dict[str, str]:
    public_ip = PUBLIC_IP or ""
    return {key: value.replace("[ip]", public_ip).replace("{public_ip}", public_ip) for key, value in query.items()}


def _parse_api_proxy_body(body: str) -> str | None:
    text = body.strip()
    if not text:
        return None
    try:
...
class ProxyRegistry:
    ...
    def load(self, pools: list[Any]) -> None:
        ...
        logger.info(
            ...
            {c: sum(1 for e in es if isinstance(e.provider, ApiProvider)) for c, es in self._by_country.items()},
        )
                ...
                "pools": [
                    {
                        ...
                        "direct_count": len(pool.get_all()) if isinstance(e.provider, DirectProvider) and pool else 0,
                        "api_cached": e.key in self._api_cache if isinstance(e.provider, ApiProvider) else False,
                    }
                    for e in entries
                ],
    ...
    def rotate(self, country: str | None = None) -> None:
        if country:
            code = country.upper()
            prefix = f"{code}:"
            self._api_cache = {k: v for k, v in self._api_cache.items() if not k.startswith(prefix)}
            self._round_robin[code] = 0
            return
        self._api_cache.clear()
        self._round_robin.clear()

    async def get(self, country: str = DEFAULT_COUNTRY) -> str | None:
        code = country.upper()
    ...
    async def _fetch_api(self, entry: PoolConfig) -> str | None:
        assert isinstance(entry.provider, ApiProvider)
        provider = entry.provider
        cached = self._api_cache.get(entry.key)
        if cached and time.time() - cached.fetched_at < PROXY_API_TTL:
            return cached.proxy
        if not provider.url:
...
async def get_proxy(country: str = DEFAULT_COUNTRY) -> str | None:
