...
def _parse_provider(raw: dict) -> Optional[DirectProvider | ApiProvider]:
    ...
    if isinstance(provider, dict):
        ...
        if ptype == 'api':
            ...
            return ApiProvider(url=str(provider.get('url') or ''), method=str(provider.get('method') or 'GET').upper(), query={str(k): str(v) for k, v in query.items()})
...
class ProxyRegistry:
    ...
    def load(self, pools: list[Any]) -> None:
        ...
        logger.info('[Proxy] loaded countries=%s direct=%s api=%s', sorted(self._by_country), {c: len(p.get_all()) for c, p in self._direct.items()}, {c: sum((1 for e in es if isinstance(e.provider, ApiProvider))) for c, es in self._by_country.items()})

    def status(self) -> dict[str, Any]:
        out: dict[str, Any] = {}
        for country, entries in sorted(self._by_country.items()):
            pool = self._direct.get(country)
            out[country] = {'pools': [{'key': e.key, 'type': e.provider.type, 'direct_count': len(pool.get_all()) if isinstance(e.provider, DirectProvider) and pool else 0, 'api_cached': e.key in self._api_cache if isinstance(e.provider, ApiProvider) else False} for e in entries], 'direct_total': len(pool.get_all()) if pool else 0}
        return out
