...
_CHANNEL_TOPICS = {'music': 'UC-9-kyTW8ZkZNDHQJ6FgpwQ'}
_SEARCH_TOPICS = {'gaming': ('gaming highlights', 'view_count'), 'news': ('tin tức việt nam hôm nay', 'upload_date'), 'sports': ('thể thao việt nam', 'view_count'), 'tech': ('review công nghệ', 'view_count'), 'beauty': ('beauty skincare review', 'view_count'), 'food': ('ẩm thực việt nam', 'view_count'), 'travel': ('du lịch việt nam', 'view_count')}
_ALL_TOPICS = list(_CHANNEL_TOPICS.keys()) + list(_SEARCH_TOPICS.keys())

@router.get('/videos/by-topic', summary='Videos by Topic')
async def videos_by_topic(topic: str=Query(..., description=f"Topic: {', '.join(_ALL_TOPICS)}"), limit: int=Query(20, ge=1, le=50), page: int=Query(1, ge=1)):
    t = topic.lower()
    if t not in _CHANNEL_TOPICS and t not in _SEARCH_TOPICS:
        raise HTTPException(status_code=400, detail=f"Unknown topic '{topic}'. Available: {', '.join(_ALL_TOPICS)}")
    ...
    async def _():
        ...
        if t in _CHANNEL_TOPICS:
            ...
            return {'topic': topic, 'source': 'channel_browse', 'playlists': result.get('playlists', []), 'featured': result.get('featured_playlist'), 'total': len(videos), 'videos': videos}
        query, sort = _SEARCH_TOPICS[t]
        results = await search_youtube(query, max_results=start + limit, sort=sort, proxy=proxy)
        return {'topic': topic, 'source': 'search', 'query': query, 'page': page, 'total': len(results), 'videos': results[start:start + limit]}
    try:
...
async def search_videos(request: Request, response: Response, q: str=Query(...), page: int=Query(1, ge=1), limit: int=Query(30, ge=1, le=50), sort: str=Query('relevance', enum=['relevance', 'upload_date', 'view_count', 'rating'])):
...
async def get_videos_live(q: str=Query(''), page: int=Query(1, ge=1), limit: int=Query(30, ge=1, le=50)):
...
async def get_videos_location(gl: str=Query(...), hl: str=Query('vi'), query: str=Query(...), max_results: int=Query(50, ge=1, le=100)):
    try:
...
async def get_comments(video_id: str, page: int=Query(1, ge=1), limit: int=Query(20, ge=1, le=100), sort: str=Query('top', enum=['top', 'newest'])):
...
async def get_comments_batch(video_ids: str=Query(..., description='Comma-separated video_ids (pick top N by view)'), limit: int=Query(20, ge=1, le=100), sort: str=Query('top', enum=['top', 'newest'])):
...
async def channel_videos(channel_id: str, page: int=Query(1, ge=1), limit: int=Query(30, ge=1, le=50)):
...
async def transcript_batch(video_ids: str=Query(..., description='Comma-separated video_ids (max 8)')):
    ...
    try:
        ...
        return ApiResponse.ok({'requested': len(ids), 'available': sum((1 for v in results.values() if v)), 'results': results})
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
