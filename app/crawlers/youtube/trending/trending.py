...
from app.config.constants import (BROWSE_ID_TRENDING, DEFAULT_TIMEOUT,
                                  DEFAULT_USER_AGENT, ENDPOINT_BROWSE,
                                  ENDPOINT_SEARCH, SORT_VIEW_COUNT,
                                  YOUTUBE_BASE_URL)
from app.config.headers import get_youtube_headers, get_youtube_html_headers
...
def extract_videos(
    items: List[Dict], rank_offset: int = 0, skip_live: bool = True
) -> List[Dict]:
    results = []
    for item in items:
...
def extract_videos_from_item(item: Dict, rank_offset: int = 0) -> List[Dict]:
    if "carouselRenderer" in item:
        ...
        if "richShelfRenderer" in content:
            return extract_videos(
                content["richShelfRenderer"].get("contents", []), rank_offset
            )
    return []
...
def _make_session(proxy: Optional[str], gl: str, hl: str) -> httpx.AsyncClient:
    headers = get_youtube_html_headers(hl=hl, gl=gl)
...
async def _api_trending(proxy, max_results, filter_params, gl, hl) -> List[Dict]:
    ...
    headers = get_youtube_headers(
        visitor_data=get_visitor_data(), client_version=get_client_version()
    )
    browse_url = get_youtube_api_url(ENDPOINT_BROWSE, api_key)
    context = get_context()
    context["client"]["gl"] = gl
    context["client"]["hl"] = hl
    payload: dict = {"context": context, "browseId": BROWSE_ID_TRENDING}
    if filter_params:
    ...
    async with create_httpx_client(proxy=proxy, headers=headers) as client:
        ...
        for item in renderers:
            if "richItemRenderer" in item:
                ...
                "richSectionRenderer" in item
                or "shelfRenderer" in item
                or "carouselRenderer" in item
            ):
            ...
            for item in items:
                if "richItemRenderer" in item:
                    ...
                    for sub in item["itemSectionRenderer"].get("contents", []):
                        collected += extract_videos_from_item(
                            sub, rank_offset=len(collected)
                        )
                elif "continuationItemRenderer" in item:
                    continuation = extract_continuation_token(item)
                if len(collected) >= max_results:
...
async def _html_trending(proxy, max_results, filter_params, gl, hl) -> List[Dict]:
    ...
    page_url = f"{YOUTUBE_BASE_URL}/feed/trending?{params}"
    ...
    if continuation and len(collected) < max_results:
        ...
        browse_url = get_youtube_api_url(ENDPOINT_BROWSE, api_key)
        ua = api_headers.get("User-Agent") or DEFAULT_USER_AGENT
        async with create_httpx_client(proxy=proxy, headers=api_headers) as client:
            while continuation and len(collected) < max_results:
                payload = {
                    "context": get_context(
                        original_url=f"{YOUTUBE_BASE_URL}/feed/trending", user_agent=ua
                    ),
...
async def _search_trending(proxy, max_results, gl, hl) -> List[Dict]:
    ...
    context = get_context(
        original_url=f"{YOUTUBE_BASE_URL}/feed/trending",
...
async def get_trending_videos(
    ...
    if proxy:
        ...
        collected, err = await _safe(
            _html_trending(proxy, max_results, filter_params, gl, hl), "html"
        )
        if err:
    ...
    if not collected:
        logger.warning(
            "🔴 [trending] all methods failed — falling back to view-count search"
        )
