...
from ..shared import (build_cookies, build_headers, create_tiki_client,
                      create_tiki_session)
from .reviews_constants import REVIEWS_EXTRA_HEADERS, REVIEWS_URL
...
def extract_review(review: Dict) -> Dict:
    images = [
        img.get("full_path") for img in review.get("images", []) if img.get("full_path")
    ]
...
def extract_reviews_page(data: Dict) -> Dict:
    ...
    return {
        ...
        "rating_summary": {
            "average": rating_summary.get("rating_avg")
            or rating_summary.get("average"),
            "count": rating_summary.get("review_count") or rating_summary.get("count"),
            "star_5": rating_summary.get("rating_count_5")
            or rating_summary.get("5", 0),
            "star_4": rating_summary.get("rating_count_4")
            or rating_summary.get("4", 0),
            "star_3": rating_summary.get("rating_count_3")
            or rating_summary.get("3", 0),
            "star_2": rating_summary.get("rating_count_2")
            or rating_summary.get("2", 0),
            "star_1": rating_summary.get("rating_count_1")
            or rating_summary.get("1", 0),
        },
...
async def get_reviews(
    ...
    async with create_tiki_client(headers, cookies, proxy=proxy) as client:
        ...
        logger.info("🟢 [tiki/reviews] GET %s → %s", resp.url, resp.status_code)
